...
## Status: phase 1 applied (surgical, low-risk)

Applied in the "Phase 1 fixes" commit, with no change to the evergreen countdown, the post-purchase progress/reveal theatre, prices, offer links, tracking, or the embedded checkouts:

- Tokens: `--ink-faint` #74816F → #5C6957 and `--gold-ink` #8A6510 → #7A5A0E on every page (all secondary text now clears 4.5:1). Hard-coded copies in the tools JS updated to match.
- Emoji cut from 27 to 11 on `index.html` (kept the "bad advice" cards and the five wealth-plan avatars); 👀 out of two headings; ⚠️ out of the tax-report alert and mistake box; emoji trust row on `upgradevip.html` now plain text.
- Kickers removed on `index.html` (11 labels + CSS); prep-kit "THAT'S WHY I BUILT..." pill is sentence case and the VIP pulsing dot is gone; `upgradevip` reveal kicker reads "Private offer" in a readable colour.
- Dead CSS pruned only where no markup, script, or Spiffy-injected element references it: ~57 rules on each ticket page, `.kicker`/`.btn-ghost-light`/`.btn-md`/`.meta-row` on the tax reports, `.kicker`/`.status-*`/`.vip-ribbon` in `confirmation.css`, `.countdown-card`/`.ba-*`/`.c-list`/`.pill-gold` on `index.html`, `.reveal`/`.flash`/`.video`/`.wrap-narrow` and the unused `hbars` renderer in the tools.
- Duplicate Spiffy loader removed from `prepkit*`, `taxreport*` and `upgradevip` (the second copy set `hideSidebar:false` last, so that effective value is kept in the remaining head snippet with a comment). Redundant font preload removed on `prepkit.html`. Tailwind leftover classes removed from `<html>`.
- Reduced motion: the before/after auto-flip on `index.html`, the deadline pulse and skeleton shimmer (`checkout.css`), the pay-button sheen (`upgradevip`), smooth scrolling and `scrollIntoView` on the tax reports/VIP prep kit/VIP upgrade now respect `prefers-reduced-motion`; the tools' global `.001ms` kill is now targeted.
- Accessibility: countdowns carry `role="timer"` and a readable label with the rolling digit strips hidden from AT; FAQ buttons have `aria-expanded` (index + both ticket pages); every Vimeo iframe has a `title`; index h2→h4 skips fixed (h4→h3, weight preserved); tax-report FAQ no longer nests `<h3>` inside `<button>`; hidden sticky bars are `visibility:hidden` so they leave the tab order; the four upgradevip reveal lines are paragraphs with `aria-hidden` toggling and no longer precede the h1; calendar buttons have action labels, the schedule says ET, and the ICS/Google events are three 10–4 sessions instead of one 54-hour block; tools ranges are wired to their visible labels, outputs have `for`, a polite live region announces the headline result, tooltips no longer sit inside `<label>` (the HSA/IRA toggles no longer flip when the "i" is tapped), reset is a real button, focus rings cover every control incl. Firefox range thumbs.
- Copy: about a dozen surgical cadence edits (index "CPAs file. They don't plan.", "He needed a sequence." run, "Real People. Real Numbers." kicker, shouted KNOWING/WHAT/WHEN/EXACT; tax-report fragment quartet, final-CTA triplet, YOUR/WON'T). Six identical `alt="Testimonial"` replaced with the reviewer and headline of each screenshot. "Anti-Inflation Investing Pack" → "Anti-Inflation Pack" to match the card and meta description.
- Assets: the three ~1 MB product PNGs replaced by WebP at identical pixel size (34/39/71 KB).
- VIP ticket order summary now shows the `VIP` badge that `checkout.css` already defined.

Deliberately NOT changed (needs a business decision or is phase 2): the evergreen timer and its "50% off" label (note: on `vipticket26.html` the discount is $50 on $197, i.e. 25%); the "$200" VIP list price on `upgradevip.html` vs "$197" on the VIP ticket page; the "DO NOT CLOSE THIS PAGE" strips, envelope reveal, confetti and progress bars; hero-metric tiles, card grids and nested cards; the dark off-brand `prechallengetraining123.html` (only its grain overlay, footer contrast and Tailwind classes were touched); the VIP `border-left` quote stripe and pill kicker; hotlinked ClickFunnels testimonial photos; `firstairbnb.com` footer links; Spiffy SDK on the confirmation pages; the `--serif` alias (no visual effect either way); shared `base.css` extraction (kept pages independently editable as requested).
